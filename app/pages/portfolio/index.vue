...
const { data: page } = await useAsyncData(`portfolio-page-${locale.value}`, () => {
  return queryCollection(`pages_${locale.value}`).path('/portfolio').first()
})
if (!page.value) {
  throw createError({
    statusCode: 404,
...
const { data: projects } = await useAsyncData(`portfolio-${locale.value}`, () => {
